...
<h1 class="h3 mb-4 text-gray-900"><?= $surat['kategori_surat']; ?><sup style="font-size:7pt;"><?=$surat['id']; ?></sup> </h1> 
		...
		<div class="card shadow">
			...
			<div class="collapse<?= ($surat['id_status'] == 10) ? "" : " show" ?>" id="collKeterangan">
				<div class="card-body">
					...
					<?php echo form_open(base_url('mahasiswa/yudisium/tambah/' . encrypt_url($surat['id'])), '') ?>
					...
					if ($fields) {
						...
						foreach ($fields as $field) {
							...
							<div class="form-group row">
								<label class="col-md-5" for="dokumen[<?= $field['id']; ?>]"><?= $kat_keterangan_surat; ?> <?= ($field['required'] == 1) ? '<sup class="badge badge-danger badge-counter">Wajib</sup>': ''; ?>
					...
					<?php if ($surat['id_status'] == 4) { ?>
						...
						<input class="btn btn-lg btn-<?= $surat['badge']; ?> btn-block" type="submit" name="submit" value="<?= ($surat['id_status'] == '4') ? "Kirim Revisi Data" : "Daftar Yudisium "?>" />

					<?php } elseif ($surat['id_status'] == 1) { ?>
						<input class="btn btn-lg btn-<?= $surat['badge']; ?> btn-block" type="submit" name="submit" value="Daftar Yudisium" />
					<?php } ?>
