...
class Yudisium extends Mahasiswa_Controller
{
	...
	{
		$data['query'] = $this->surat_model->get_surat_bycat($this->session->userdata('user_id'),'6');
		$data['title'] = 'Yudisium';
	...
	public function daftar()
	{
		...
		$kat_surat = $this->db->select('id')->from('kat_keterangan_surat')->where(['id_kategori_surat' => $id, 'aktif' => 1])->get()->result_array();
		...
		redirect(base_url('mahasiswa/yudisium/tambah/' . encrypt_url($insert_id)));
		...
		$surat =	$this->surat_model->get_detail_surat($id_surat);
			...
			if ($this->form_validation->run() == FALSE) {
				...
				// kirim notifikasi
					...
					if ($set_status) { 
						redirect(base_url('mahasiswa/yudisium/tambah/' . encrypt_url($id_surat)));
					}
