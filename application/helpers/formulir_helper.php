...
function generate_form_field($field_id, $pengajuan_id, $pengajuan_status, $fungsi_upload, $jenis_pengajuan_id)
{
	...
	$value = $CI->db->select('fv.value, fv.verifikasi, fv.catatan')->from('keterangan_surat fv')
		->where(array('id_kat_keterangan_surat' => $field_id, 'id_surat' => $pengajuan_id))
		->get()->row_array();
...
function generate_keterangan_surat($field_id, $id_pengajuan, $pengajuan_status, $jenis_pengajuan_id)
{
	...
	$CI = &get_instance();
	$field = $CI->db->select('kks.*')->from('kat_keterangan_surat kks')
		->where(array('kks.id' => $id))
		->get()->row_array();

	$field_key = ($field) ? $field['key'] : '';

	$fields = $CI->db->select('ks.value, ks.verifikasi, ks.catatan')->from('keterangan_surat ks')
		->where(array('id_kat_keterangan_surat' => $field_id, 'id_surat' => $id_pengajuan))
		->get()->row_array();
