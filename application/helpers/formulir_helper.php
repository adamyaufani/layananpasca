...
?>
  ...
  <?php } elseif ($fields['type'] == 'multi_select_anggota') { ?>
    ...
    <select class="js-data-example-ajax form-control form-control-lg <?= $fields['key']; ?> form-control <?= (form_error('dokumen[' . $id . ']')) ? 'is-invalid' : ''; ?> <?= (($fields['verifikasi'] == 0) && ($pengajuan_status == 4)) ? 'is-invalid' : ''; ?>" <?= ($pengajuan_status == 1 && $fields['verifikasi'] == 0 || $pengajuan_status == 4 && $fields['verifikasi'] == 0) ? "" : "disabled"; ?> value="<?= (validation_errors()) ? set_value('dokumen[' . $id . ']') :  $fields['value'];  ?>" name="dokumen[<?= $id; ?>][]" multiple>
      <?php
      if ($pengajuan_status == 1 && $fields['verifikasi'] == 0 || $pengajuan_status == 4 && $fields['verifikasi'] == 0) {
      } else {
      ?>
...
?>
                ...
                foreach ($media as $row) { ?>
                  <div class="col-md-3 media mb-3">
                    <figure class="img-thumbnail " style="min-height:200px; border:1px solid #ddd;text-align:center;">
                      ...
                      <p class="nama-file" style="display:block;bottom:0; padding:5px 0;"><?php get_file_name($row['file']); ?></p>
                    </figure>
                  </div>
                <?php } ?>
              </div>
  ...
  <script>
    ...
    $(document).on('click', '.link', function() {
      ...
      var namafile = $(this).next('.nama-file').text(); /*ambil id gambar */
      ...
      $("#" + value).next("p.nama-file").html(namafile).show();
      // $("." + value).show();
...
function get_meta_value($key, $id_pengajuan, $file)
{
  ...
  if ($file == true) {
    $media = $CI->db->select("file")->from('Tr_Media')->where(array('id' => $value['value']))->get()->row_array()['file'];
